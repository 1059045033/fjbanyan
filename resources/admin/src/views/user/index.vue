<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="上下班时间" min-width="150px">
        <template slot-scope="{row}">

          <router-link :to="'/user/working-time/'+row.id">
            <div v-if="row.working_time"  @click="updateWorkingTime(row)">
              <p v-for="item in row.working_time">{{ item.name }}: {{ item.start_time }} ~ {{ item.end_time }}</p>
            </div>
          </router-link>
