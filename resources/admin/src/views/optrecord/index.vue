...
<!--      <el-input v-model="listQuery.name" placeholder="公司名字" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />-->
...
<!--      </el-button>-->
    ...
    >
      ...
      <el-table-column label="操作人" min-width="60px">
        <template slot-scope="{row}">
          <span>{{ row.opt_user_name }}</span>
        </template>
      </el-table-column>

      <el-table-column label="IP" min-width="60px">
        <template slot-scope="{row}">
          <span>{{ row.ip }}</span>
        </template>
      </el-table-column>
...
<script>
import { fetchRecodLogsAllList } from '@/api/optrecord'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchRecodLogsAllList(this.listQuery).then(response => {
