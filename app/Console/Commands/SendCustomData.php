...
class SendCustomData extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'sendData:Custom {--start=} {--end=}';
     ...
     * @var string
     */
    protected $description = '发送自定义时间的数据';
    ...
    public function handle()
    {
        ...
        # 获取参数 日期参数
        $start_day = empty($this->option('start')) ? date('Y-m-d'):$this->option('start');
        $end_day = empty($this->option('end')) ? date('Y-m-d'):$this->option('end');

        DB::transaction(function () use ($start_day,$end_day){
            $companies = DB::table('companies')->pluck('name','id')->toArray();
            $regions   = DB::table('work_regions')->pluck('name','id')->toArray();
            $time = time();

            # 日期的起止时间戳
            $start = Carbon::parse($start_day)->startOfDay()->timestamp;
            $end   = Carbon::parse($end_day)->endOfDay()->timestamp;
            ...
            $params = [
                'to_name' => "尊敬的领导",
                'message' => '福州共享单车 '.date('Y-m').'打包数据【'.$start_day.'~'.$end_day.'】',//福州共享单车 2022-05 打包数据
                'type'    => 'month',
                'data' => $taskurls
            ];

            Mail::send('email',['params'=>$params],function($message) use($start_day,$end_day) {
                $to = ['190507753@qq.com','359448144@qq.com','1073043199@qq.com'];
                $message ->to($to)->subject(date('Y-m')."打包数据【'.$start_day.'~'.$end_day.'】");
            });
